# -*- coding: utf-8 -*-
"""Apply chosen replacement images (B2a,B3c,F2a) into raw/; re-hunt C1a phinisi and build a C1a options sheet."""
...
for q in ["Pinisi Komodo","Phinisi Labuan Bajo","Komodo National Park tourist boat","Kelor island boat Komodo","liveaboard boat Raja Ampat","Pinisi Bira boat"]:
    for c in search(q):
        ...
        seen.add(c["title"]); c1a.append((q,c))
        if len(c1a)>=6: break
    if len(c1a)>=6: break
for i,(q,c) in enumerate(c1a): dl(c["thumb"], os.path.join(OPT,f"C1a_{i}.img"))
opt["C1a_new"]=[{"q":q,**c} for q,c in c1a]
...
print("C1a opts:", [(q,c["title"].replace("File:","")[:36]) for q,c in c1a])
...
CW,IH,LH=300,200,42; COLS=3; rows=max(1,(len(c1a)+COLS-1)//COLS)
sh=Image.new("RGB",(COLS*CW,rows*(IH+LH)),(20,20,24)); dr=ImageDraw.Draw(sh)
for i,(q,c) in enumerate(c1a):
    r,cc=divmod(i,COLS); x0,y0=cc*CW,r*(IH+LH)
    p=os.path.join(OPT,f"C1a_{i}.img")
    ...
    dr.text((x0+6,y0+IH+2),f"C1a #{i}",font=font(17),fill=(255,225,130))
    dr.text((x0+6,y0+IH+22),c["title"].replace("File:","")[:34],font=font(11),fill=(170,170,182))
sh.save(os.path.join(BUILD,"sheets","c1a.jpg"),"JPEG",quality=84)
print("wrote c1a.jpg", sh.size)
