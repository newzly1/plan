...
印尼（巴厘岛及周边）6 人旅行选点投票项目。核心产物是自包含网页 `webpage/index.html`，供 6 人在浏览器里投票参考（纯 `localStorage`，无后端）。
...
## 构建与部署
- 重新构建（改内容/样式/交互后）：`cd webpage && python3 build.py` → 生成自包含 `index.html`（只用 Python 标准库）
- 改哪个文件：内容 → `content.py`；样式 → `style.css`；投票/交互 → `app.js`
- 部署到线上（国内可直连，链接不变）：`cd webpage && tcb hosting deploy index.html index.html -e plan-d0gstt7r6507aa319`
